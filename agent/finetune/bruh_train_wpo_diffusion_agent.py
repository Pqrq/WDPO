...
class TrainWPOAgent(TrainAgent):
    def __init__(self, cfg):
        ...
        self.replay_buffer = ReplayBuffer(
            ...
            self.action_dim * cfg.horizon_steps,
    ...
    def run(self):
        ...
        while self.itr < self.n_train_itr:
            ...
            for step in range(self.n_steps):
                ...
                # Select Action (Attention: Action sampling takes place here!)
                with torch.no_grad():
                    ...
                    #samples = self.model(flat_obs_torch, deterministic=False) # Old one
                    samples = self.model({"state": flat_obs_torch}, deterministic=False) # New one (gemini)
